...
enrichr.files <- grep(pattern =".*table.up", enrichr.files,ignore.case = T, value = T)
enrichr.files
...
enrichr.data.list <- lapply(enrichr.files, function(f) {
  ...
  set_names(stringr::str_remove_all(basename(enrichr.files), "\\_table.up.txt"))
...
```
### Get enriched terms with Adj.P.value <= 0.001
```{r}
...
out.file
write.table(df.up, file = out.file, sep ="\t")
```
```
