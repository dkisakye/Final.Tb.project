...
Load data file
```{r}
data.file <- "Taileux.datasets/02_tailleux_18hrs_a.Rmd.tt.DC.2020_05_22.txt"
...
genes.up <- df3 %>%
  ...
  genes.up.uniq.Taileux.dendritic <- sort(genes.up$SYMBOL)
  head(genes.up.uniq.Taileux.dendritic)
  write(genes.up.uniq.Taileux.dendritic, file = "genes.up.uniq.Taileux.dendritic")
```
...
genes.down <- df3 %>%
  ...
  genes.down.uniq.Taileux.dendritic <- sort(genes.down$SYMBOL)
  head(genes.down.uniq.Taileux.dendritic)
  write(genes.down.uniq.Taileux.dendritic, file = "genes.down.uniq.Taileux.dendritic")
```
...
parameter.table.up.Taileux.dendritic <- matrix(rep(NA, (length(p.value.cutoffs) * length(fc.cutoffs.up))),
                                 ncol = length(fc.cutoffs.up),
                                 dimnames = list(
                                as.character(p.value.cutoffs),
                                as.character(fc.cutoffs.up)),
                                )
parameter.table.up.Taileux.dendritic

fc.cutoffs.down <- fc.cutoffs.up*-1
fc.cutoffs.down
parameter.table.down.Taileux.dendritic <- parameter.table.up.Taileux.dendritic %>%
  set_colnames(fc.cutoffs.down)

parameter.table.down.Taileux.dendritic
```
...
for (i in seq_along(fc.cutoffs.up)) {
  for (j in seq_along(p.value.cutoffs)) {
    parameter.table.up.Taileux.dendritic[i,j] <- df3 %>%
...
parameter.table.up.Taileux.dendritic
write(parameter.table.up.Taileux.dendritic, file = "parameter.table.up.Taileux.dendritic")
```
Down-regulated genes a function of parameters
```{r}
for (i in seq_along(fc.cutoffs.down)) {
  for (j in seq_along(p.value.cutoffs)) {
    parameter.table.down.Taileux.dendritic[i,j] <- df3 %>%
      filter(adj.P.Val <= p.value.cutoffs[i]) %>%
...
parameter.table.down.Taileux.dendritic
write(parameter.table.down.Taileux.dendritic, file = "parameter.table.down.Taileux.dendritic")
```
