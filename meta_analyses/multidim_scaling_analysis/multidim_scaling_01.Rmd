...
```{r}
rna.seq.data.files$GSE67472_MP <-"../source_data/rna_seq/GSE67427/GSE67427_table_s2.txt"
stopifnot(file.exists(rna.seq.data.files$GSE67472_MP))
#rna.seq.data.list <-list()
rna.seq.data.list$GSE67472_MP<- read.table(file = rna.seq.data.files$GSE67472_MP, header = T, sep = "\t", stringsAsFactors = FALSE)  %>% 
#str(rna.seq.data.list$GSE67472) #data.frame':	12728 obs. of  140 variables:
 ...
 select(matches(c("name","Rv.18.logFC"), ignore.case = TRUE)) %>%
  ...
  summarise(logFC = median(logFC), .groups="drop") %>% 
  as.data.frame(., stringsAsFactors=FALSE)
 head(rna.seq.data.list$GSE67472_MP)
```

```{r}
# WITHOUT GROUPING / AGGREGATING:
dim(rna.seq.data.list$GSE67472_MP) #  12724     2
sapply(rna.seq.data.list$GSE67472_MP, class) # "character"   "numeric"
range(rna.seq.data.list$GSE67472_MP$logFC) # -4.155479  8.205439
sum(rna.seq.data.list$GSE67472_MP$logFC == 0) # 0
sum(duplicated(rna.seq.data.list$GSE67472_MP$Gene.symbol)) # 0
# dups <- rna.seq.data$GSE64179$Gene.symbol[duplicated(rna.seq.data$GSE64179$Gene.symbol)]
# subset(rna.seq.data$GSE64179, Gene.symbol %in% dups) %>% 
#   arrange(Gene.symbol, logFC)
# head(rna.seq.data$GSE64179)
...
ggsave(gg.mds, filename = out.file, height = 10, width = 15, units = "cm", useDingbats=)
out.file
```
