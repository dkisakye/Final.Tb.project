---
title: "GSE148731: Generate toptable for MF2 data"
author: "A. Nohturfft"
date: "`r Sys.Date()`"
output:
  html_notebook:
...
```{r results="asis"}
data.file <- list.files(pattern=".*MF2.*xlsx$")
data.file
...
```{r}
list.files()
tt.mf1 <- read.table(file="GSE148731_toptable_MF1_01.Rmd.2020_07_09.txt", sep="\t", header=T, stringsAsFactors = F)
```

```{r}
mf1 <- select(tt.mf1, Gene.symbol, logFC) %>% 
  dplyr::rename(MF1 = logFC)
mf2 <- select(top.table, Gene.symbol, logFC, MF2 = logFC)
mf12 <- merge(x=mf1, y=mf2, by="Gene.symbol", all=FALSE) #%>% 
  # tidyr::pivot_longer(-Gene.symbol) %>% 
  # set_colnames(c("Gene", "Dataset", "logFC"))
head(mf12)
```

```{r}
with(mf12, cor(MF1, MF2))
# 0.4952674
```


```{r}
gg1 <- ggplot(mf12, aes(x=MF1, y=MF2)) + geom_point()
gg1
```
