...
### Filter rows: Two NA max  
```{r}
two.na.max.count <- as.matrix(merged.df) %>% 
  apply(., 1, function(x) {  # 1 = select rows
    n <- sum(is.na(x))
    n
  })
class(two.na.max.count) # integer
length(two.na.max.count) # 30687
merged.df.filt.na <- merged.df[two.na.max.count <= 2,]
dim(merged.df.filt.na)# 11873     8  vs  8626    12 (one NA max)
 dim(merged.df)    #  30687     8
```
```{r}
cat("Number of rows with max two NA: <b>", comma(nrow(merged.df.filt.na)), "</b> (",
...
panel.cor <- function(x, y, digits = 3, prefix = "", cex.cor, ...)
{
 ...
 merged.df %>% 
  ...
  pairs(., col = alpha("black", 0.2), pch=20, cex =0.1, upper.panel = panel.cor,
        xaxt='n', yaxt='n',
