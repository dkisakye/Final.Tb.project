...
```{r}
write.table(x, file="delete.txt", sep="\t", row.names = F, col.names = T)
```
...
```{r}
col <- list(Technology = c("Microarray" = "lightgreen", "RNA-seq" = "yellow"),
...
heat.cols <- colorRampPalette(RColorBrewer::brewer.pal(7, "YlGnBu"))(256) %>% rev
...
```{r}
out.file.map.down <- paste("data_out", stringr::str_replace(this.script,"\\.[Rr][Mm][Dd]$", ".map.down.pdf"), sep="/")
out.file.map.down
```

```{r}
# pdf(file = out.file.map.down, paper = "a4")
# heat.cols <- colorRampPalette(RColorBrewer::brewer.pal(7, "YlGnBu"))(256) %>% rev
# ComplexHeatmap::Heatmap(
  # mx.down.100, name = "Rank",
  # col = heat.cols,
...
ggsave(filename = out.file.map.down, last_plot(), height = 21, width = 29, units = "cm", useDingbats=)

# filename = out.file.map.down, plot = last_plot(), height = 21, width = 29, units = "cm", )
...
# "YlOrRd", "YlOrBr", "YlGnBu"*, "PuBuGn", "YlGn", "RdPu"-, "PuRd"-, "BuGn"
# "Blues", "Greens", "Purples", "Oranges"
heat.cols <- colorRampPalette(RColorBrewer::brewer.pal(8, "YlGnBu"))(256) %>% rev
ComplexHeatmap::Heatmap(
  mx.up.100, name = "Rank",
  col = heat.cols,
  top_annotation = ha)
...
```{r}
pdf(file = out.file.map.up, paper = "a4")
heat.cols <- colorRampPalette(RColorBrewer::brewer.pal(7, "YlGnBu"))(256) %>% rev
ComplexHeatmap::Heatmap(
  mx.up.100, name = "Rank",
  col = heat.cols,
