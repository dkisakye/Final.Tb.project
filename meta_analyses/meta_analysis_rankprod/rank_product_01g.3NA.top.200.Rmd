...
10. Generate heatmaps of top 200 regulated genes  
...
panel.cor <- function(x, y, digits = 3, prefix = "", cex.cor, ...)
{
     ...
     text(0.5, 0.5, txt, cex=1.0, col="blue", font = 3)
}
merged.df %>% 
  set_colnames(names(all.data.list)) %>% 
  pairs(., col = alpha("black", 0.3), pch=20, upper.panel = panel.cor,
        xaxt='n', yaxt='n',
        text.panel = function(x,y,lab,cex,font) {text(x,y,lab, cex=2, font=2)})
```
...
```{r}
pdf(file=out.file, width = 6.7, height=4)
merged.df %>% 
  set_colnames(names(all.data.list)) %>% 
  pairs(., col = alpha("black", 0.2), pch=20, upper.panel = panel.cor,
        xaxt='n', yaxt='n',
        text.panel = function(x,y,lab,cex,font) {text(x,y,lab, cex=0.5, font=2)})
dev.off()
```
...
### Heatmap: 200 most downregulated genes  
```{r}
# nrow(subset(rank.sum.up, P.Val.Adj <= 0.05)) # 594
# nrow(subset(rank.sum.up, P.Val.Adj <= 0.01)) # 317
down.200 <- dplyr::arrange(rank.sum.down, P.Val.Adj, Rank.Sum) %>% 
  head(.,200) %>% .$Gene
 # tail(down.200)
mx.down.200 <- ranks.down.mx.2[down.200,] %>% 
  set_colnames(names(all.data.list))
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), "_DC", " (DC)")
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), "_MP", " (MP)")
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), ".MF1", " (MF1)")
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), ".MF2", " (MF2)")
# dim(mx.down.200) # 200   5
head(mx.down.200)
```
### Save top 200 upregulated genes
```{r}
out.file <- paste("data_out", stringr::str_replace(this.script,"\\.[Rr][Mm][Dd]$", ".down.200.txt"), sep="/")

out.file
cat(down.200, file = out.file, sep = "\n")
```



```{r}
down.200.df <- as.data.frame(t(mx.down.200))
# dim(down.200.df) # 5 200

row.names(down.200.df) <- names(all.data.list)
...
down.200.df$Technology[down.200.df$Dataset %in% names(rna.seq.data.list)] <- "RNA-seq"
down.200.df$Cell.type[down.200.df$Dataset %in% c("GSE64179", "A-BUGS-23_DC", "GSE34151")] <- "Dendritic cells"
down.200.df[,1:6]
```
...
ha.down <- ComplexHeatmap::HeatmapAnnotation(
  Technology = down.200.df$Technology,
  Cell.type = down.200.df$Cell.type,
  col=col.down
)
rm(col.down)
```
...
get.heatmap(mx.down.200, heat.cols, ha.down)
```
...
```{r}
pdf(file = out.file.map.down, paper = "a4", width= 6, height= 8.5)
get.heatmap(mx.down.200, heat.cols, ha.down)
dev.off()
```


### Heatmap: 200 most upregulated genes  
```{r}
up.200 <- dplyr::arrange(rank.sum.up, P.Val.Adj, Rank.Sum) %>% 
  head(.,200) %>% .$Gene
# tail(up.200)
mx.up.200 <- ranks.up.mx.2[up.200,] %>% 
  set_colnames(names(all.data.list))
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), "_DC", " (DC)")
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), "_MP", " (MP)")
colnames(mx.down.200) <- str_replace(colnames(mx.down.200), ".MF1", " (MF1)")
...
### Save top 200 upregulated genes
```{r}
out.file <- paste("data_out", stringr::str_replace(this.script,"\\.[Rr][Mm][Dd]$", ".up.200.txt"), sep="/")

out.file
cat(up.200, file = out.file, sep = "\n")

```


```{r}
up.200.df <- as.data.frame(t(mx.up.200))
# dim(down.200.df) # 5 200
row.names(up.200.df) <- names(all.data.list)
...
up.200.df <- as.data.frame(t(mx.up.200)) %>% 
  ...
  dplyr::mutate(Cell.type = "Macrophages") %>%
  dplyr::select(Dataset, Technology, Cell.type, dplyr::everything())
up.200.df$Technology[up.200.df$Dataset %in% names(rna.seq.data.list)] <- "RNA-seq"
up.200.df$Cell.type[up.200.df$Dataset %in% c("GSE64179", "A-BUGS-23_DC","GSE34151")] <- "Dendritic cells"
...
ha.up <- ComplexHeatmap::HeatmapAnnotation(
  Technology = up.200.df$Technology,
  Cell.type = up.200.df$Cell.type,
  col=col.up
)
heat.cols <- colorRampPalette(RColorBrewer::brewer.pal(8, "Oranges"))(256) %>% rev
get.heatmap(mx.up.200, heat.cols, ha.up)
```
...
```{r}
pdf(file = out.file.map.up, paper = "a4", width = 6 , height = 8.5)
get.heatmap(mx.up.200, heat.cols, ha.up)
dev.off()
```
